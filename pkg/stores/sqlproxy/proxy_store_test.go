...
func TestNewProxyStore(t *testing.T) {
	...
	tests = append(tests, testCase{
		...
		test: func(t *testing.T) {
			...
			cf.EXPECT().CacheFor([][]string{{`id`}, {`metadata`, `state`, `name`}}, gomock.Any(), &tablelistconvert.Client{ResourceInterface: ri}, attributes.GVK(&nsSchema), false).Return(c, nil)
	...
	tests = append(tests, testCase{
		...
		test: func(t *testing.T) {
			...
			scc.EXPECT().SetColumns(context.Background(), &nsSchema).Return(nil)
			cg.EXPECT().TableAdminClient(nil, &nsSchema, "", &WarningBuffer{}).Return(ri, nil)
			cf.EXPECT().CacheFor([][]string{{`id`}, {`metadata`, `state`, `name`}}, gomock.Any(), &tablelistconvert.Client{ResourceInterface: ri}, attributes.GVK(&nsSchema), false).Return(factory.Cache{}, fmt.Errorf("error"))
...
func TestReset(t *testing.T) {
	...
	tests = append(tests, testCase{
		...
		test: func(t *testing.T) {
			...
			cg.EXPECT().TableAdminClient(nil, &nsSchema, "", &WarningBuffer{}).Return(ri, nil)
			cf.EXPECT().CacheFor([][]string{{`id`}, {`metadata`, `state`, `name`}}, gomock.Any(), &tablelistconvert.Client{ResourceInterface: ri}, attributes.GVK(&nsSchema), false).Return(nsc2, nil)
